...
SELECT
	...
	*
	REPLACE (
		...
		enum_range(NULL::"${opt.schema}.exact_times_v")[exact_times::integer + 1] AS exact_times
		` : ``}
	),
...
CREATE UNIQUE INDEX "${opt.schema}.frequencies_unique" ON "${opt.schema}.frequencies" (
	trip_id,
	-- As of v1.0.0, DuckDB does not support UNIQUE indexes on INTERVAL columns yet, so we cast to INTEGER.
	(start_time::string)
);

CREATE INDEX "${opt.schema}.frequencies_trip_id" ON "${opt.schema}.frequencies" (trip_id);
CREATE INDEX "${opt.schema}.frequencies_exact_times" ON "${opt.schema}.frequencies" (exact_times);
`)
