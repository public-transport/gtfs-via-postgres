...
CREATE TABLE "${opt.schema}".feed_info (
	...
	feed_lang TEXT NOT NULL,
		-- todo: as of DuckDB v1.0.0, "subqueries prohibited in CHECK constraints"
		-- CONSTRAINT valid_feed_lang CHECK (
		-- 	EXISTS (SELECT * FROM valid_lang_codes WHERE lang_code = feed_lang)
		-- ),
	-- todo: wait for https://github.com/duckdb/duckdb/issues/604
	-- > There are still a few places where collations […] are currently not considered:
	-- > - […]
	-- > - Support for collations in indexes
	-- FOREIGN KEY (feed_lang) REFERENCES "${opt.schema}".valid_lang_codes,
	default_lang TEXT,
		-- todo: as of DuckDB v1.0.0, "subqueries prohibited in CHECK constraints"
		-- CONSTRAINT valid_default_lang CHECK (
		-- 	EXISTS (SELECT * FROM valid_lang_codes WHERE lang_code = default_lang)
		-- ),
	-- todo: see above
	-- FOREIGN KEY (default_lang) REFERENCES "${opt.schema}".valid_lang_codes,
