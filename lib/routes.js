...
CREATE TABLE "${opt.schema}".routes (
	...
	agency_id TEXT ${opt.routesWithoutAgencyId ? '' : `FOREIGN KEY (agency_id) REFERENCES "${opt.schema}".agency,`}
...
`)
	...
	workingState.nrOfRowsByName.set(
		'routes',
		(await db[GET](`SELECT count(route_id) AS count FROM "${opt.schema}".routes`))[0].count,
	)
}
