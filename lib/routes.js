...
const importData = async (db, pathToRoutes, opt, workingState) => {
	...
	const withAgencyFKey = !opt.routesWithoutAgencyId && !exactly1Agency
...
CREATE TABLE routes (
	...
	-- As of DuckDB v1.3.0, a foreign key constraint does not enforce non-NULL values.
	agency_id TEXT ${withAgencyFKey ? `NOT NULL` : ''},
	${withAgencyFKey
		? ''
