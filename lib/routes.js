...
const importData = async (db, pathToRoutes, opt, workingState) => {
...
CREATE TABLE "${opt.schema}.routes" (
	...
	agency_id TEXT,
	${opt.routesWithoutAgencyId || exactly1Agency
		? ''
		: `FOREIGN KEY (agency_id) REFERENCES "${opt.schema}.agency",`
	}
...
CREATE INDEX "${opt.schema}.routes_route_short_name" ON "${opt.schema}.routes" (route_short_name);
`)

	workingState.nrOfRowsByName.set('routes', await queryNumberOfRows(db, 'routes', opt))
}
