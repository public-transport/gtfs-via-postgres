...
CREATE TABLE "${opt.schema}.stops" (
	...
	stop_timezone TEXT,
	FOREIGN KEY (stop_timezone) REFERENCES "${opt.schema}.valid_timezones",
		...
		SELECT
			...
			*
			...
			REPLACE (
				...
				enum_range(NULL::"${opt.schema}.location_type_val")[location_type + 1] AS location_type,
				` : ``}
				${has_wheelchair_boarding ? `
				-- Casting an integer to an enum (using the index) is currently not possible, so we have to compute the availability index by hand using enum_range().
				-- DuckDB array/list indixes are 1-based.
				enum_range(NULL::"${opt.schema}.wheelchair_boarding_val")[ifnull(wheelchair_boarding, 0) + 1] AS wheelchair_boarding
				` : ``}
			)
		FROM read_csv(
			'${pathToStops}',
...
CREATE INDEX "${opt.schema}.stops_parent_station" ON "${opt.schema}.stops" (parent_station);
${opt.stopsLocationIndex ? `CREATE INDEX "${opt.schema}.stops_stop_loc" ON "${opt.schema}.stops" (stop_loc);` : ''}
`)
