...
CREATE TABLE _translations_ref_cols (
	...
	record_id_col TEXT NOT NULL
		CONSTRAINT valid_record_id_col CHECK (
			...
			OR
			column_exists(table_name, record_id_col)
		),
	record_sub_id_col TEXT
		CONSTRAINT valid_record_sub_id_col CHECK (
			...
			OR
			column_exists(table_name, record_sub_id_col)
		)
);
...
-- https://gtfs.org/documentation/schedule/reference/#translationstxt
INSERT INTO _translations_ref_cols (
	table_name,
	record_id_col,
	record_sub_id_col
) VALUES
...
AS $$
	...
	BEGIN
		IF col_b_name IS NULL THEN
			...
			', 'main', table_name, col_a_name, col_a_value)
			INTO STRICT result;
			RETURN result;
		ELSE
			EXECUTE format('
			...
			', 'main', table_name, col_a_name, col_a_value, col_b_name, col_b_value)
			INTO STRICT result;
			RETURN result;
		END IF;
	END;
...
CREATE OR REPLACE FUNCTION is_valid_translation_ref(
	_table_name TEXT,
	_field_name TEXT,
	_record_id TEXT,
	_record_sub_id TEXT,
...
CREATE TABLE translations (
	...
	field_name TEXT NOT NULL
		CONSTRAINT valid_field_name CHECK (
			NOT table_exists(table_name)
			OR
			column_exists(table_name, field_name)
		),
...
SELECT
	...
	platform_code
FROM stops s
LEFT JOIN translations stop_n_t ON (
...
LEFT JOIN translations stop_d_t ON (
...
LEFT JOIN translations stop_u_t ON (
...
CREATE OR REPLACE VIEW routes_translated AS
SELECT
	-- almost all columns, duh
	-- todo: find a way to use all columns without explicitly enumerating them here
	route_id,
...
LEFT JOIN translations route_d_t ON (
...
LEFT JOIN translations route_u_t ON (
...
CREATE OR REPLACE VIEW trips_translated AS
SELECT
	-- almost all columns, duh
	-- todo: find a way to use all columns without explicitly enumerating them here
	trip_id,
	...
	bikes_allowed
FROM trips t
LEFT JOIN translations trip_s_t ON (
...
LEFT JOIN translations trip_h_t ON (
