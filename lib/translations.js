...
const _srcTableRefSql = (schema, table_name) => {
	return `"${schema}"."_translations_${table_name}"`
}

const _srcTablesSql = (pathToTranslations, schema, table_name, translationRef) => {
	const {
		record_id_column,
	...
	const srcTableRef = _srcTableRefSql(schema, table_name)
...
strictEqual(
	_srcTablesSql('foo/trans.txt', 'gtfs', 'feed_info', {
		record_id_column: null,
		record_sub_id_column: null, record_sub_id_column_type: null,
	}),
	`\
CREATE TABLE "gtfs"."_translations_feed_info" (
...
INSERT INTO "gtfs"."_translations_feed_info"
SELECT
...
strictEqual(
	_srcTablesSql('foo/trans.txt', 'gtfs', 'calendar_dates', {
		record_id_column: 'service_id',
		record_sub_id_column: 'date', record_sub_id_column_type: 'DATE',
	}),
	`\
CREATE TABLE "gtfs"."_translations_calendar_dates" (
	...
	record_sub_id DATE NOT NULL,
		...
		REFERENCES "gtfs"."calendar_dates" (
			"service_id"
			, "date"
		),
	field_name TEXT NOT NULL, -- todo: validate via all_columns helper view
...
INSERT INTO "gtfs"."_translations_calendar_dates"
SELECT
	record_id,
	record_sub_id,
	field_name,
...
const _selectToBeMergedSql = (schema, table_name, translationRef) => {
	const {
		record_id_column,
...
strictEqual(
	_selectToBeMergedSql('gtfs', 'agency', {
		record_id_column: 'agency_id',
		record_sub_id_column: null, record_sub_id_column_type: null,
	}),
	`\
	...
	FROM "gtfs"."_translations_agency"
`,
	'_selectToBeMergedSql with agency.txt',
)
strictEqual(
	_selectToBeMergedSql('gtfs', 'calendar_dates', {
		record_id_column: 'service_id',
		record_sub_id_column: 'date', record_sub_id_column_type: 'DATE',
	}),
	`\
	...
	FROM "gtfs"."_translations_calendar_dates"
`,
	'_selectToBeMergedSql with calendar_dates.txt',
)

const _translatedSql = (schema, table_name, translatedCols) => {
	const tableRef = `"${schema}"."${table_name}"`
	...
	.map(([col, translationRef]) => {
		...
		const hasSubCol = record_sub_id_column !== null
		const subColRef = hasSubCol ? `"${record_sub_id_column}"` : null
		const srcTableRef = _srcTableRefSql(schema, src_table_name)
...
LEFT JOIN ${srcTableRef} ${transRef} ON (
	...
	return `\
CREATE VIEW "${schema}"."${table_name}_translated" AS
SELECT
...
}
{
	...
	strictEqual(
		_translatedSql('gtfs', 'agency', new Map([
			['agency_name', agencyRef],
			['agency_url', agencyRef],
		])),
		`\
CREATE VIEW "gtfs"."agency_translated" AS
SELECT
	-- todo: fall back to feed_info.feed_lang?
...
FROM "gtfs"."agency" data
LEFT JOIN "gtfs"."_translations_agency" "trans_agency_name" ON (
	"trans_agency_name".field_name = 'agency_name'
	AND data."agency_id" = "trans_agency_name".record_id
	
)
LEFT JOIN "gtfs"."_translations_agency" "trans_agency_url" ON (
...
}
{
	...
	strictEqual(
		_translatedSql('gtfs', 'calendar_dates', new Map([
			['foo', calendarDatesRef],
			['b-a-r', calendarDatesRef],
		])),
		`\
CREATE VIEW "gtfs"."calendar_dates_translated" AS
SELECT
	-- todo: fall back to feed_info.feed_lang?
...
FROM "gtfs"."calendar_dates" data
LEFT JOIN "gtfs"."_translations_calendar_dates" "trans_foo" ON (
	"trans_foo".field_name = 'foo'
	AND data."service_id" = "trans_foo".record_id
	AND data."date" = "trans_foo".record_sub_id
)
LEFT JOIN "gtfs"."_translations_calendar_dates" "trans_b-a-r" ON (
...
}
{
	...
	strictEqual(
		_translatedSql('gtfs', 'feed_info', new Map([
			['foo', {
				...feedInfoRef,
		...
		`\
CREATE VIEW "gtfs"."feed_info_translated" AS
SELECT
	-- todo: fall back to feed_info.feed_lang?
	"trans_foo".language AS "foo_lang",
	"trans_b-a-r".language AS "b-a-r_lang",
...
FROM "gtfs"."feed_info" data
LEFT JOIN "gtfs"."_translations_some-other-table" "trans_foo" ON (
	"trans_foo".field_name = 'foo'
	
	
)
LEFT JOIN "gtfs"."_translations_feed_info" "trans_b-a-r" ON (
	"trans_b-a-r".field_name = 'b-a-r'
...
const importData = async (db, pathToTranslations, opt, workingState) => {
	...
	for (const [table_name, translationRef] of supportedTranslationRefs.entries()) {
		await db[RUN](_srcTablesSql(pathToTranslations, opt.schema, table_name, translationRef))
...
CREATE VIEW "${opt.schema}".translations AS
...
`)
	...
	for (const [table_name, translatedCols] of preTranslatedColumns) {
		await db[RUN](_translatedSql(opt.schema, table_name, translatedCols))
	}

	// *_translated for tables/views made up by gtfs-via-postgres
	{
		await db[RUN](_translatedSql(opt.schema, 'arrivals_departures', new Map([
			['route_short_name', routesRef],
			['route_long_name', routesRef],
			['trip_headsign', tripsRef],
