	...
	const [{
		...
		SELECT
			EXISTS(
				SELECT *
				FROM columns
				WHERE column_name = 'length'
			) AS has_length,
			EXISTS(
				SELECT *
				FROM columns
				WHERE column_name = 'traversal_time'
			) AS has_traversal_time,
			EXISTS(
				SELECT *
				FROM columns
				WHERE column_name = 'stair_count'
			) AS has_stair_count,
			EXISTS(
				SELECT *
				FROM columns
				WHERE column_name = 'max_slope'
			) AS has_max_slope,
			EXISTS(
				SELECT *
				FROM columns
				WHERE column_name = 'min_width'
			) AS has_min_width,
			EXISTS(
				SELECT *
				FROM columns
				WHERE column_name = 'signposted_as'
			) AS has_signposted_as,
			EXISTS(
				SELECT *
				FROM columns
				WHERE column_name = 'reversed_signposted_as'
			) AS has_reversed_signposted_as
`)
...
CREATE CAST ("${opt.schema}".pathway_mode_v AS text) WITH INOUT AS IMPLICIT;
...
CREATE TABLE "${opt.schema}".pathways (
	...
	pathway_mode "${opt.schema}".pathway_mode_v NOT NULL,
...
INSERT INTO "${opt.schema}".pathways
