...
const populateImportMetadata = async (db, _, opt) => {
	...
	await db[RUN](`\
CREATE OR REPLACE FUNCTION "${opt.schema}.gtfs_data_imported_at" (a)
AS (
	'${new Date(opt.importStart).toISOString()}'::timestamp with time zone
);

CREATE OR REPLACE FUNCTION "${opt.schema}.gtfs_via_duckdb_version" ()
AS (
	'${pkg.version}'::text
);

CREATE OR REPLACE FUNCTION "${opt.schema}.gtfs_via_duckdb_options" ()
AS (
