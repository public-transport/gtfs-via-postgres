...
CREATE TABLE "${opt.schema}.service_days" (
	service_id TEXT NOT NULL,
	date TIMESTAMP NOT NULL,
	PRIMARY KEY (service_id, date)
);

INSERT INTO "${opt.schema}.service_days" 
SELECT
...
CREATE UNIQUE INDEX "${opt.schema}.service_days_unique_service_id_date" ON "${opt.schema}.service_days" (service_id, date);

CREATE INDEX "${opt.schema}.service_days_service_id" ON "${opt.schema}.service_days" (service_id);
CREATE INDEX "${opt.schema}.service_days_date" ON "${opt.schema}.service_days" (date);
-- apparently the unique index (service_id, date) doesn't speed up queries
CREATE INDEX "${opt.schema}.service_days_service_id_date" ON "${opt.schema}.service_days" (service_id, date);
`)
}
