...
const importData = async (db, opt, workingState) => {
	await db[RUN](`\
...
CREATE UNIQUE INDEX service_days_unique_service_id_date ON "${opt.schema}".service_days (service_id, date);

CREATE INDEX service_days_service_id ON "${opt.schema}".service_days (service_id);
CREATE INDEX service_days_date ON "${opt.schema}".service_days (date);
-- apparently the unique index (service_id, date) doesn't speed up queries
CREATE INDEX service_days_service_id_date ON "${opt.schema}".service_days (service_id, date);
`)
}
