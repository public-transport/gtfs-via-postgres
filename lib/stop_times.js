...
FROM (
	SELECT
		...
		trips.service_id,
		stop_times.stop_sequence, -- todo [breaking]: rename to from_stop_sequence, add to_stop_sequence
	...
	INNER JOIN stops to_stations ON to_stops.parent_station = to_stations.stop_id
) trips
JOIN (
	SELECT *
	FROM service_days
