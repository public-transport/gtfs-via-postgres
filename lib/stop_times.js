...
SELECT
	...
	*
	REPLACE (
		...
		enum_range(NULL::"${opt.schema}.pickup_drop_off_type")[drop_off_type + 1] AS drop_off_type,
		enum_range(NULL::"${opt.schema}.pickup_drop_off_type")[pickup_type + 1] AS pickup_type
		${has_timepoint ? `,enum_range(NULL::"${opt.schema}.timepoint_v")[timepoint + 1] AS timepoint` : ''}
	),
...
CREATE INDEX "${opt.schema}.stop_times_trip_id" ON "${opt.schema}.stop_times" (trip_id);
CREATE INDEX "${opt.schema}.stop_times_stop_id" ON "${opt.schema}.stop_times" (stop_id);
CREATE INDEX "${opt.schema}.stop_times_stop_sequence_consec" ON "${opt.schema}.stop_times" (stop_sequence_consec);
CREATE INDEX "${opt.schema}.stop_times_trip_id_stop_sequence_consec" ON "${opt.schema}.stop_times" (trip_id, stop_sequence_consec);
...
-- CREATE INDEX "${opt.schema}.stop_times_arrival_time" ON "${opt.schema}.stop_times" (arrival_time);
-- CREATE INDEX "${opt.schema}.stop_times_departure_time" ON "${opt.schema}.stop_times" (departure_time);

-- todo: use materialized view once DuckDB supports that
-- see also https://github.com/duckdb/duckdb/discussions/3638
CREATE TABLE "${opt.schema}.largest_arr_dep_time" AS
WITH
	largest_departure_time AS (
		SELECT departure_time
		FROM "${opt.schema}.stop_times" stop_times
		WHERE EXISTS (
			SELECT *
			FROM "${opt.schema}.trips" trips
			JOIN "${opt.schema}.service_days" service_days ON service_days.service_id = trips.service_id
...
WITH stop_times_based AS NOT MATERIALIZED (
	...
	FROM (
		...
		JOIN "${opt.schema}.stops" stops ON s.stop_id = stops.stop_id
		LEFT JOIN "${opt.schema}.stops" stations ON stops.parent_station = stations.stop_id
		JOIN "${opt.schema}.trips" trips ON s.trip_id = trips.trip_id
		JOIN "${opt.schema}.routes" routes ON trips.route_id = routes.route_id
		LEFT JOIN "${opt.schema}.agency" agency ON (
		...
		JOIN "${opt.schema}.service_days" service_days ON trips.service_id = service_days.service_id
	)
...
FROM (
	SELECT
		...
		frequencies_row
	FROM stop_times_based
	JOIN "${opt.schema}.frequencies" frequencies ON frequencies.trip_id = stop_times_based.trip_id
...
-- AS $$
-- 	SELECT *
-- 	FROM "${opt.schema}.arrivals_departures" arrivals_departures
...
WITH stop_times_based AS NOT MATERIALIZED (
	...
	FROM (
		...
		FROM "${opt.schema}.trips" trips
		LEFT JOIN "${opt.schema}.routes" routes ON trips.route_id = routes.route_id
		LEFT JOIN "${opt.schema}.agency" agency ON (
			-- The GTFS spec allows routes.agency_id to be NULL if there is exactly one agency in the feed.
			-- Note: We implicitly rely on other parts of the code base to validate that agency has just one row!
			-- It seems that GTFS has allowed this at least since 2016:
		...
		LEFT JOIN "${opt.schema}.stop_times" stop_times ON trips.trip_id = stop_times.trip_id
	...
	JOIN (
		SELECT *
		FROM "${opt.schema}.service_days" service_days
...
-- AS $$
-- 	SELECT *
-- 	FROM "${opt.schema}.connections" connections
-- 	WHERE trip_id = decode(from_base64(split_part(id, ':', 1)))
-- 	AND "date" = decode(from_base64(split_part(id, ':', 2)))::timestamp
