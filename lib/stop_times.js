...
CREATE INDEX ON "${opt.schema}".stop_times (stop_id, trip_id);
...
WITH stop_times_based AS NOT MATERIALIZED (
	SELECT
		...
		CASE WHEN
			stops.wheelchair_boarding IS NULL
			OR stops.wheelchair_boarding = 
'no_info_or_inherit'
		THEN
			coalesce(stations.wheelchair_boarding, 'no_info_or_inherit')
		ELSE
	...
	FROM (
		SELECT
			...
			from_stations.stop_name as from_station_name,
   			CASE WHEN
	  			from_stops.wheelchair_boarding IS NULL
	  			OR from_stops.wheelchair_boarding = 'no_info_or_inherit'
	  		THEN
	 			coalesce(from_stations.wheelchair_boarding, 'no_info_or_inherit')
	 		ELSE
				from_stops.wheelchair_boarding
			END AS from_stop_wheelchair_boarding,
			...
			to_stations.stop_name as to_station_name,
   			CASE WHEN
	  			to_stops.wheelchair_boarding IS NULL
	  			OR to_stops.wheelchair_boarding = 'no_info_or_inherit'
	  		THEN
	 			coalesce(to_stations.wheelchair_boarding, 'no_info_or_inherit')
	 		ELSE
				to_stops.wheelchair_boarding
			END AS to_stop_wheelchair_boarding
		FROM "${opt.schema}".trips
