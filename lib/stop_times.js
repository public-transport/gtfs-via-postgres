...
WITH stop_times_based AS NOT MATERIALIZED (
	SELECT
		...
		CASE WHEN stops.wheelchair_boarding IS NULL OR stops.wheelchair_boarding = 
'no_info_or_inherit' THEN CASE WHEN stations.wheelchair_boarding IS NULL THEN 
'no_info_or_inherit' ELSE stations.wheelchair_boarding END ELSE 
stops.wheelchair_boarding END AS wheelchair_boarding
	FROM (
