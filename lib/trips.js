...
const beforeAll = (opt) => `\
DROP TYPE IF EXISTS ${opt.schema}.wheelchair_accessibility;
...
DROP TYPE IF EXISTS ${opt.schema}.bikes_allowance;
...
CREATE TABLE ${opt.schema}.trips (
	...
	block_id TEXT,
	shape_id TEXT,
	${opt.tripsWithoutShapeId ? '' : `FOREIGN KEY (shape_id) REFERENCES ` + opt.schema + ` (shape_id),`}
