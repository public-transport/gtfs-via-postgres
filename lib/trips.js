...
SELECT
	...
	*
	REPLACE (
		...
		enum_range(NULL::"${opt.schema}.wheelchair_accessibility")[wheelchair_accessible + 1] AS wheelchair_accessible,
		` : ``}
		${has_bikes_allowed ? `
		-- Casting an integer to an enum (using the index) is currently not possible, so we have to compute the availability index by hand using enum_range().
		-- DuckDB array/list indixes are 1-based.
		enum_range(NULL::"${opt.schema}.bikes_allowance")[bikes_allowed + 1] AS bikes_allowed
		` : ``}
	)
FROM read_csv(
	'${pathToTrips}',
