...
const convertGtfsToSql = async (pathToDb, files, opt = {}) => {
	...
	db[GET] = promisify(db.all.bind(db))
...
`)
	...
	for (const name of order) {
		...
		if (task.file) {
			await importData(db, task.file, opt, workingState)
		} else {
			await importData(db, opt, workingState)
		}
	}

	console.error(workingState.nrOfRowsByName) // todo
