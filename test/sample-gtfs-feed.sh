...
path_to_db="$(mktemp -d -t gtfs)/sample-gtfs-feed.duckdb"
...
arr1=$(duckdb -csv -noheader -c "$query" | head -n 1)
if [[ "$arr1" != "1553993700" ]]; then
	echo "invalid 1st t_arrival: $arr1" 1>&2
	exit 1
fi

arr2=$(duckdb -csv -noheader -c "$query" | head -n 2 | tail -n 1)
...
freq_arr_dep1=$(duckdb -csv -noheader -c "$arrs_deps_b_downtown_on_working_days" | head -n 1)
...
freq_arr_dep2=$(duckdb -csv -noheader -c "$arrs_deps_b_downtown_on_working_days" | head -n 2 | tail -n 1)
...
dst1=$(duckdb -csv -noheader -c "$connection_during_dst" | head -n 1)
...
lvl1=$(duckdb -csv -noheader -c "$airport_levels" | head -n 1)
...
pw1=$(duckdb -csv -noheader -c "$airportPathway" | head -n 1)
...
exact1=$(duckdb -csv -noheader -c "$timepoint_exact" | head -n 1)
...
airport_entrance_translation=$(duckdb -csv -noheader -c "$stops_translations")
...
bikes_allowed_arrs_deps_rows="$(duckdb -csv -noheader -c "$bikes_allowed_arrs_deps_query")"
...
stops_translated_rows="$(duckdb -csv -noheader -c "$stops_translated_query")"
