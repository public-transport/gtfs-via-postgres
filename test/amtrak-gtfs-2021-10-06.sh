...
version=$(duckdb -csv -noheader -c "SELECT split_part(\"amtrak.gtfs_via_duckdb_version\"(), '.', 1)" "$path_to_db" | tail -n 1)
if [[ "$version" != "4" ]]; then
	echo "invalid gtfs_via_duckdb_version(): $version" 1>&2
	exit 1
fi

# todo
# fMin=$(duckdb -csv -noheader -c "SELECT \"amtrak.dates_filter_min\"('2021-11-27T13:45:00-06')" "$path_to_db" | tail -n 1)
