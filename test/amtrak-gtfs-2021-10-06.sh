...
arrN=$(duckdb -csv -noheader -c "$query" | tail -n 1)
if [[ "$arrN" != "1638038700" ]]; then
	echo "invalid 2nd t_arrival: $arrN" 1>&2
	exit 1
fi

version=$(duckdb -csv -noheader -c "SELECT split_part(amtrak.gtfs_via_postgres_version(), '.', 1)" | tail -n 1)
if [[ "$version" != "4" ]]; then
	echo "invalid gtfs_via_postgres_version(): $version" 1>&2
	exit 1
fi

fMin=$(duckdb -csv -noheader -c "SELECT amtrak.dates_filter_min('2021-11-27T13:45:00-06')" | tail -n 1)
...
acelaStat=$(duckdb -csv -noheader -c "$acelaStatQuery" | tail -n 1)
...
acelaPhillyStat=$(duckdb -csv -noheader -c "$acelaPhillyStatQuery" | tail -n 1)
...
nrOfActiveTrips=$(duckdb -csv -noheader -c "$nrOfActiveTripsQuery" | tail -n 1)
